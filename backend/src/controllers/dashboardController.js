...
export const getAdminDashboard = async (req, res) => {
  try {
    ...
    const [
      ...
      Product.find({ stockQuantity: { $gt: 0, $lte: 10 }, isArchived: false }).select("name stockQuantity reorderLevel").limit(10),
      Product.countDocuments({ stockQuantity: 0, isArchived: false }),
